...
function generateAnonymousData() {
    ...
    return { name: `${adj} ${noun}`, avatar: avatar };
}
...
function uploadFileAndCreatePost(text, file, pollData) {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            // Get the secure URL and the file type from Cloudinary's response
            createPost(text, response.secure_url, response.resource_type);
        } else {
    ...
    xhr.onerror = function() {
        ...
        alert("File upload failed. Please check your network connection.");
        resetPostCreator();
    };
...
function createPost(text, mediaUrl, mediaType, pollData) {
    ...
    const postObject = {
        ...
        questionText: text,
        mediaUrl: mediaUrl,
        mediaType: mediaType, // Cloudinary provides 'image' or 'video'
...
function renderPostCard(doc) {
    ...
    if (post.poll) {
        ...
        const optionsHTML = post.poll.options.map(option => {
            ...
            return `<div class="poll-option ${isVoted ? 'voted' : ''}" data-post-id="${postId}" data-option-text="${option.text}"><div class="poll-option-fill" style="width: ${percentage}%;"></div><div class="poll-option-info"><span class="poll-option-text">${option.text}</span><span class="poll-option-percent">${Math.round(percentage)}%</span></div></div>`;
        }).join('');
...
document.body.addEventListener('click', e => {
    ...
    if (postCard && !e.target.closest('.upvote-btn, .action-btn, .poll-option')) { showPostDetailView(postCard.dataset.postId); return; }
